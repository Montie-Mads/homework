...
//   res.send(`${numberOfBottles} bottles of beer on the wall. Take one down and <a href="/bottles."/>`)
// });
...
// Display a link that reduces the bottle by 1
// If there are no bottles left, restart!
app.get('api/bottles/:numberOfBottles', function (req, res) {
  let numberOfBottles = Number(req.params.numberOfBottles) || 99;
...
app.get('/bottles/:numberOfBottles', function (req, res) {
  ...
  let numberOfBottles = Number(req.params.numberOfBottles) || 99;
  let newNumberOfBottles = -1

  res.send({ "bottles": numberOfBottles, "Next": `http://localhost:3001/api/${newNumberOfBottles}` });
